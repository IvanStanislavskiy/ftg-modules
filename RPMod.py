...
class RPMod(loader.Module):
	...
	async def client_ready(self, client, db):
		...
		if not self.db.get('RPMod', 'rpcomands', False):
			comands = {'чмок': 'чмокнул', 'кусь': 'кусьнул', 'пиц': 'радостные звуки питсы', 'поцеловать': 'поцеловал',
			           'ррр': 'злые звуки питсы', 'выебать': 'выебал', 'трахнуть': 'трахнул', 'выпороть': 'выпорол',
			           'шлепнуть': 'шлепнул', 'отлизать': 'отлизал у', 'прижать': 'прижал', 'погладить': 'погладил',
			           'да.': 'пизда', 'где.': 'в пизде', 'нет.': 'пидора ответ',
			           'бывает.': 'ну это пиздец конечно на самом деле', 'мрр.': 'мурчание питсы',
			           'ррррр.': 'злая питса', 'обнять': 'обнял'}
					...
					if not dict == type(emj):
						...
						for x in emj.values():
							...
							if not x or not x.strip():
								await message.edit(f"<b>Пустое значение в словаре для эмодзи? Да ты гений.</b>")
								return
							if x.isalpha() or x.isspace() or x.isdigit() or x in string.punctuation:
								await message.edit(
								...
								return
							if len(lst) > 3:
								await message.edit(f"<b>Было введено более 3 эмодзи в словаре для эмодзи. </b>")
								return
						...
						for x in emj:
							...
							if int(len(key_len)) > 1:
								...
								return
							if not x or not x.strip():
								await message.edit(f"<b>Пустой ключ в словаре для эмодзи? Да ты умничка.</b>")
								return
				com = eval(comands)
				if dict == type(com):
					if len(com) == 0:
					...
					for x in com.values():
						if not x or not x.strip():
							await message.edit(f"<b>Пустое значение в словаре для команд? Не сегодня, мой золотой.</b>")
							return
					if count_emoji == 1:
		...
		if not args:
			...
			if a.id in ex_copy:
				...
				try:
					...
					name = a.first_name
				await message.edit(f'<i>Чат <b><u>{name}</u></b>(<code>{a.id}</code>) добавлен в исключения.</i>')
		elif args.isdigit():
			args = args.strip()
			args = int(args)
			if args in ex_copy:
