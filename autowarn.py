...
class AutoWarnforMuteMod(loader.Module):
    ...
    async def watcher(self, message):
        ...
        if (chatid == -1001387610194) and (fromid == 1626675239):
            if 'длилась:' in msg:
                x = await message.client.get_participants(-1001387610194, limit=100, filter=ChannelParticipantsBanned)
                for i in x:
                    muted.append(i.id)
                for mid in muted:
                ...
                for usr in message.entities:
                    if hasattr(usr, 'user_id'):
                        if usr.user_id in muted:
                            ...
                            await message.client.send_message(-1001430533627, f"!warn {str(usr.user_id)} Игра под мутом")
                            await message.client.send_message(1361873517, f"<b>[Игра под мутом/Warn] </b>Выдал варн <a href=\"tg://user?id={str(usr.user_id)}\">{username}</a> ибо играет с мутом\n\n{timestamp}")
