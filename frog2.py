...
@loader.tds
class Frog2Mod(loader.Module):
    """Frog"""
    strings = {'name': 'helpmv'}

    async def watcher(self, message):
        if message.sender_id in {465029324}:
        ...
        try:
                    ...
                    async with message.client.conversation(chat) as conv:
                        try:
                            ...
                            if "Можно отправиться" in response.text:
                                ...
                                if message.chat_id in KW:
                                    response = conv.wait_event(events.NewMessage(incoming=True, from_users=465029324, chats=message.chat_id))
                                    await sleep (10)
            ...
            if message.sender_id in {1124824021}:
                ...
                if "Ivan " + name in message.message:
                    await sleep (10)
                    await message.respond('реанимировать жабу')
                    await sleep (10)
                    await message.respond('отправиться за картой')
